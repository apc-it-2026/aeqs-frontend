...
namespace SjeMES_QCM_Ex
{
    public partial class FGT_Required_List : MaterialForm
    {
        ...
        {
            MonthlyFGTReport(dt_Month.Text, cb_fgt_result.Text,txt_art.Text);
        }

        public void MonthlyFGTReport(string Month,string FGT_Result,string Article)
        {
            try
            {
        ...
        public void AutogenerateColumns(DataTable dt)
        {
            ...
            {
                Name = "SALES_ORDER",
                HeaderText = "SALES_ORDER",
                DataPropertyName = "SALES_ORDER"
            });
     ...
     .Where(row =>
         ...
         (
             ...
             !string.IsNullOrWhiteSpace(row["TEST_TYPE"]?.ToString()) &&
             !string.IsNullOrWhiteSpace(row["SHOE_SIZE"]?.ToString())
         )
     );
    ...
    : dt.Clone();
        ...
        public static string ConvertFGTRequestToWhatsApp(DataRow dr)
        {
            ...
            string Sales_Order = dr["SALES_ORDER"]?.ToString();
            ...
            sb.AppendLine($"🆔 Sales Order   : {Sales_Order}");
        ...
        private void DataGridView1_DataBindingComplete(object sender, DataGridViewBindingCompleteEventArgs e)
        {
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                // ✅ NEW: Make only specific cells readonly
                row.Cells["SHOE_SIZE"].ReadOnly = hasProdSendDate;
                row.Cells["TEST_TYPE"].ReadOnly = hasProdSendDate;
       ...
       private DataTable GetSizes(string PO_Number,string WorkCenter)
        ...
        private void Btn_excel_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                // ✅ Create only TEXT columns (NO clone)
                foreach (DataGridViewColumn col in dataGridView1.Columns)
                {
                    if (col.Name != "CONFIRM")
                    {
                        tempGrid.Columns.Add(new DataGridViewTextBoxColumn()
                        {
                            Name = col.Name,
                ...
                foreach (DataGridViewRow row in dataGridView1.Rows)
                {
                    ...
                    foreach (DataGridViewColumn col in dataGridView1.Columns)
                    {
                        if (col.Name != "CONFIRM")
                        {
                            ...
                            tempGrid.Rows[index].Cells[col.Name].Value =
                ...
                // Export
                ExportExcels.Export("Lab_Report.xls", tempGrid);
